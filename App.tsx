  ...
  <nav className="bg-blue-600 text-white p-4 shadow-lg flex justify-between items-center sticky top-0 z-50">
    <div className="flex items-center gap-3">
      ...
      <div>
        ...
        <span className="text-[10px] uppercase tracking-widest opacity-70 font-bold">Smart OMR Reader AI</span>
      </div>
    </div>
    <div className="text-sm bg-blue-700/50 backdrop-blur-sm px-3 py-1 rounded-full border border-white/10 font-medium">v2.0 (AI Enabled)</div>
  </nav>
);
  ...
  const processKeyImage = async (file: File) => {
    ...
    try {
      ...
      if (data.error) {
        ...
        if (data.isAuthError) handleOpenKeySelector();
        return;
      }
    ...
    <div className="min-h-screen pb-20">
      ...
      <main className="max-w-4xl mx-auto p-4 md:p-8">
            ...
            {processingProgress.total > 0 && (
              <div className="w-full max-w-xs space-y-4">
                ...
                <div className="w-full h-3 bg-white/10 rounded-full overflow-hidden">
                  <div className="h-full bg-blue-500 transition-all duration-500" style={{ width: `${(processingProgress.current / processingProgress.total) * 100}%` }}></div>
                </div>
              </div>
            )}
          </div>
          ...
          <section className="space-y-6 animate-fadeIn">
            ...
            {subjects.length === 0 ? (
              <div className="bg-white border-2 border-dashed border-gray-200 rounded-[2rem] p-16 text-center text-gray-400 shadow-sm">
                ...
                <p className="text-lg font-bold text-gray-500">ยังไม่มีข้อมูล</p>
                <p className="text-sm font-medium opacity-70 mt-1">เริ่มต้นสร้างรายวิชาใหม่เพื่อเปิดใช้งานระบบตรวจ AI</p>
              </div>
            ) : (
        ...
        {currentStep === AppStep.SETUP_SUBJECT && (
          <section className="bg-white p-8 sm:p-10 rounded-[2.5rem] shadow-xl border border-gray-100 max-w-lg mx-auto animate-fadeIn relative overflow-hidden">
            ...
            <h2 className="text-2xl font-black mb-1 text-gray-800">สร้างรายวิชาใหม่</h2>
            <p className="text-gray-400 text-xs mb-8 font-bold uppercase tracking-widest">Create Exam Subject</p>
            <form onSubmit={(e) => {
              e.preventDefault();
              ...
              <div className="space-y-6">
                <div>
                  <label className="block text-xs font-black text-gray-500 uppercase tracking-widest mb-2">ระบุชื่อรายวิชา</label>
                  <input required name="name" type="text" placeholder="เช่น กลางภาค วิชาวิทยาศาสตร์" className="w-full px-5 py-4 rounded-2xl border border-gray-200 focus:ring-4 focus:ring-blue-100 focus:border-blue-500 outline-none transition-all font-medium" />
                </div>
                <div>
                  <label className="block text-xs font-black text-gray-500 uppercase tracking-widest mb-2">ระบุจำนวนข้อสอบ (1-100)</label>
                  <div className="relative">
                    <input required name="count" type="number" min="1" max="100" placeholder="จำนวนข้อ" className="w-full px-5 py-4 rounded-2xl border border-gray-200 focus:ring-4 focus:ring-blue-100 focus:border-blue-500 outline-none transition-all font-bold text-lg pr-12" />
        ...
        {currentStep === AppStep.CALIBRATE_KEY && activeSubject && (
          <section className="space-y-10 animate-fadeIn text-center">
            <div className="max-w-md mx-auto">
              <h2 className="text-3xl font-black mb-3 text-gray-800 leading-tight">ระบุเฉลย</h2>
              <p className="text-gray-500 mb-10 text-sm font-medium">อัปโหลดภาพแผ่นเฉลยเพื่อให้ AI ช่วยตรวจจับคำตอบอัตโนมัติ</p>
              ...
              <div className="relative group overflow-hidden rounded-[3rem] h-64 shadow-2xl shadow-blue-50 mb-10">
                ...
                <div className="border-4 border-dashed border-blue-50 group-hover:border-blue-300 rounded-[3rem] p-8 transition-all duration-300 bg-white pointer-events-none h-full flex flex-col items-center justify-center">
                  ...
                  <span className="text-lg font-black text-gray-700 block">อัปโหลดภาพแผ่นเฉลย</span>
                  <span className="text-[10px] text-gray-300 mt-2 font-black uppercase tracking-widest italic">AI Detection: ON</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-8 rounded-[3rem] shadow-xl border border-gray-50 text-left animate-slideUp">
              <div className="flex items-center gap-3 mb-8">
                ...
                <h3 className="text-xl font-black text-gray-800">ตารางเฉลย ({activeSubject.totalQuestions} ข้อ)</h3>
              </div>
        ...
        {currentStep === AppStep.SCAN_STUDENTS && activeSubject && (
          <section className="space-y-10 animate-fadeIn text-center">
            <div className="max-w-lg mx-auto">
              <h2 className="text-3xl font-black mb-3 text-gray-800">สแกนตรวจแผ่นคำตอบ</h2>
              <p className="text-gray-500 mb-10 text-sm font-medium leading-relaxed">ถ่ายรูปเพื่อตรวจทันที หรือเลือกไฟล์ภาพจำนวนมากเพื่อประมวลผลพร้อมกัน</p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="relative group overflow-hidden rounded-[2.5rem] h-64 shadow-xl hover:shadow-2xl transition-all duration-300">
                  ...
                  <div className="border-2 border-blue-50 group-hover:border-blue-400 rounded-[2.5rem] p-8 transition bg-white pointer-events-none h-full flex flex-col items-center justify-center">
                    ...
                    <span className="text-xl font-black text-gray-800">เปิดกล้องถ่ายภาพ</span>
                  </div>
                </div>
                ...
                <div className="relative group overflow-hidden rounded-[2.5rem] h-64 shadow-xl hover:shadow-2xl transition-all duration-300">
                  ...
                  <div className="border-2 border-gray-50 group-hover:border-blue-300 rounded-[2.5rem] p-8 transition bg-white pointer-events-none h-full flex flex-col items-center justify-center">
                    ...
                    <span className="text-xl font-black text-gray-800">เลือกไฟล์จากเครื่อง</span>
                  </div>
                </div>
              </div>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50/50 border-b border-gray-100 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">
                    ...
                    <th className="px-8 py-5">ชื่อนักเรียน</th>
                    <th className="px-8 py-5 text-center">คะแนน</th>
                    <th className="px-8 py-5 text-right">ลบ</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {activeSubject.results.map((r, idx) => (
                    <tr key={r.id} className="hover:bg-blue-50/20 transition-all duration-300">
                      <td className="px-8 py-5 font-black text-gray-800">{r.studentNumber}</td>
                      <td className="px-8 py-5 font-bold text-gray-700">{r.studentName}</td>
                      <td className="px-8 py-5 text-center">
                         <span className={`inline-block px-4 py-1 rounded-full font-black text-xl bg-blue-50 text-blue-700`}>
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-xl border-t border-gray-100 p-4 text-center z-40 flex flex-col sm:flex-row items-center justify-between px-8">
        <div className="flex items-center gap-2">
           <span className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-300">TODSUAB • SMART OMR AI</span>
        </div>
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 font-black text-[10px] uppercase tracking-widest px-4 py-1.5 rounded-full shadow-sm text-green-600 bg-green-50">
            <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>
            AI Engine: Active
          </div>
          {!isApiKeySelected && (
            <button onClick={handleOpenKeySelector} className="text-[10px] font-bold text-blue-600 hover:underline underline-offset-4">
              Switch API Key
            </button>
          )}
        </div>
